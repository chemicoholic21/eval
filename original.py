...
def create_resume_prompt(job_desc, resume, criteria):
    ...
    return f"""Analyze the provided resume against the job role and criteria. Output a JSON object evaluating 'Education and Company Pedigree', 'Skills & Specialties', 'Work Experience', 'Basic Contact Details', and 'Educational Background Details' on specified scales, including a justification for each. Also, provide a summary for 'Input on Job Specific Criteria'. You are an AI hiring manager. Follow the instructions exactly. Only use the information provided in the input. Never invent or assume details. If information is missing, state this in your justification. Always output in the specified JSON format. Candidate should have worked or is working in a startup.
*Input:*
- Job Role: {job_desc}
- Resume: {resume}
...
def create_interview_prompt(job_desc, transcript, criteria):
    # This prompt is from your first script
    return f"""Analyze the interview transcript to assess the candidate's 'Demonstrated Technical/Role Knowledge/Skills'.You are an AI interviewer. Only use the information provided in the transcript and job role. Never invent, assume, or add details. If information is missing, state this in your justification. Always output in the specified JSON format. Candidate should have worked or is working in a startup.
...
#     """Summarizes the candidate's evaluation based on various inputs.
# *Input:*
# - Job Role: {job_desc}
# - Interview Transcript: {transcript}
# - Job-Specific Criteria: {criteria}
...
- "Manual Intervention": ONLY when there are conflicting signals, borderline scores, or insufficient data/ interview transcript to make a clear decision, also only if the job description and resume tech skills are matching you can go for "Manual Intervention", else "Reject". Make "Manual Intervention" as rare as possible.
...
def main():
    ...
    try:
        ...
        if os.path.exists("PE.csv"):
            df = pd.read_csv("PE.csv")
            print(f"📄 Successfully loaded 'PE.csv' with {len(df)} rows.")
        else:
            excel_files = [f for f in os.listdir('.') if f.endswith('.xlsx')]
            if not excel_files:
                raise FileNotFoundError("No 'PE .csv' or .xlsx file found.")
    ...
    # --- Save Output ---
    output_filename = "PE posteval.csv"
    try:
